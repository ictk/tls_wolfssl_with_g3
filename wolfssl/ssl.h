...
#endif
//START NEO_API_DEC
NEO_API const char*  wc_ecc_get_name(int curve_id);
NEO_API int ecc_projective_add_point(ecc_point* P,ecc_point* Q,ecc_point* R,mp_int* a,mp_int* modulus,mp_digit mp);
NEO_API int ecc_projective_dbl_point(ecc_point* P,ecc_point* R,mp_int* a,mp_int* modulus,mp_digit mp);
NEO_API int wc_ecc_make_key(WC_RNG* rng,int keysize,ecc_key* key);
NEO_API int wc_ecc_make_key_ex(WC_RNG* rng,int keysize,ecc_key* key,int curve_id);
NEO_API int wc_ecc_make_pub(ecc_key* key,ecc_point* pubOut);
NEO_API int wc_ecc_check_key(ecc_key* key);
NEO_API int wc_ecc_is_point(ecc_point* ecp,mp_int* a,mp_int* b,mp_int* prime);
NEO_API int wc_ecc_shared_secret(ecc_key* private_key,ecc_key* public_key,byte* out,word32* outlen);
NEO_API int wc_ecc_shared_secret_gen(ecc_key* private_key,ecc_point* point,byte* out,word32 * outlen);
NEO_API int wc_ecc_shared_secret_ex(ecc_key* private_key,ecc_point* point,byte* out,word32 * outlen);
NEO_API int wc_ecc_sign_hash(const byte* in,word32 inlen,byte* out,word32 * outlen,WC_RNG* rng,ecc_key* key);
NEO_API int wc_ecc_sign_hash_ex(const byte* in,word32 inlen,WC_RNG* rng,ecc_key* key,mp_int * r,mp_int * s);
NEO_API int wc_ecc_verify_hash(const byte* sig,word32 siglen,const byte* hash,word32 hashlen,int* stat,ecc_key* key);
NEO_API int wc_ecc_verify_hash_ex(mp_int * r,mp_int * s,const byte* hash,word32 hashlen,int* stat,ecc_key* key);
NEO_API int wc_ecc_init(ecc_key* key);
NEO_API int wc_ecc_init_ex(ecc_key* key,void* heap,int devId);
NEO_API void wc_ecc_free(ecc_key* key);
NEO_API int wc_ecc_set_flags(ecc_key* key,word32 flags);
NEO_API int wc_ecc_set_curve(ecc_key* key,int keysize,int curve_id);
NEO_API int wc_ecc_is_valid_idx(int n);
NEO_API int wc_ecc_get_curve_idx(int curve_id);
NEO_API int wc_ecc_get_curve_id(int curve_idx);
NEO_API int wc_ecc_get_curve_size_from_id(int curve_id);
NEO_API int wc_ecc_get_curve_idx_from_name(const char* curveName);
NEO_API int wc_ecc_get_curve_size_from_name(const char* curveName);
NEO_API int wc_ecc_get_curve_id_from_name(const char* curveName);
NEO_API int wc_ecc_get_curve_id_from_params(int fieldSize,const byte* prime,word32 primeSz,const byte* Af,word32 AfSz,const byte* Bf,word32 BfSz,const byte* order,word32 orderSz,const byte* Gx,word32 GxSz,const byte* Gy,word32 GySz,int cofactor);
NEO_API ecc_point*  wc_ecc_new_point();
NEO_API ecc_point*  wc_ecc_new_point_h(void* h);
NEO_API void wc_ecc_del_point(ecc_point* p);
NEO_API void wc_ecc_del_point_h(ecc_point* p,void* h);
NEO_API int wc_ecc_copy_point(ecc_point* p,ecc_point * r);
NEO_API int wc_ecc_cmp_point(ecc_point* a,ecc_point * b);
NEO_API int wc_ecc_point_is_at_infinity(ecc_point * p);
NEO_API int wc_ecc_mulmod(mp_int* k,ecc_point * G,ecc_point * R,mp_int* a,mp_int* modulus,int map);
NEO_API int wc_ecc_mulmod_ex(mp_int* k,ecc_point * G,ecc_point * R,mp_int* a,mp_int* modulus,int map,void* heap);
NEO_API int wc_ecc_export_x963(ecc_key* key,byte* out,word32* outLen);
NEO_API int wc_ecc_export_x963_ex(ecc_key* key,byte* out,word32* outLen,int compressed);
NEO_API int wc_ecc_import_x963(const byte* in,word32 inLen,ecc_key* key);
NEO_API int wc_ecc_import_x963_ex(const byte* in,word32 inLen,ecc_key* key,int curve_id);
NEO_API int wc_ecc_import_private_key(const byte* priv,word32 privSz,const byte* pub,word32 pubSz,ecc_key* key);
NEO_API int wc_ecc_import_private_key_ex(const byte* priv,word32 privSz,const byte* pub,word32 pubSz,ecc_key* key,int curve_id);
NEO_API int wc_ecc_rs_to_sig(const char* r,const char* s,byte* out,word32* outlen);
NEO_API int wc_ecc_sig_to_rs(const byte* sig,word32 sigLen,byte* r,word32* rLen,byte* s,word32* sLen);
NEO_API int wc_ecc_import_raw(ecc_key* key,const char* qx,const char* qy,const char* d,const char* curveName);
NEO_API int wc_ecc_import_raw_ex(ecc_key* key,const char* qx,const char* qy,const char* d,int curve_id);
NEO_API int wc_ecc_export_private_only(ecc_key* key,byte* out,word32* outLen);
NEO_API int wc_ecc_export_public_raw(ecc_key* key,byte* qx,word32* qxLen,byte* qy,word32* qyLen);
NEO_API int wc_ecc_export_private_raw(ecc_key* key,byte* qx,word32* qxLen,byte* qy,word32* qyLen,byte* d,word32* dLen);
NEO_API int wc_ecc_export_point_der(const int curve_idx,ecc_point* point,byte* out,word32* outLen);
NEO_API int wc_ecc_import_point_der(byte* in,word32 inLen,const int curve_idx,ecc_point* point);
NEO_API int wc_ecc_size(ecc_key* key);
NEO_API int wc_ecc_sig_size(ecc_key* key);
NEO_API int wc_ecc_get_oid(word32 oidSum,const byte* * oid,word32* oidSz);
NEO_API int wc_AesCbcEncrypt(Aes* aes,byte* out,const byte* in,word32 sz);
NEO_API int wc_AesCbcDecrypt(Aes* aes,byte* out,const byte* in,word32 sz);
...
NEO_API int neo_api_verify_mac(int ssl_ret);
NEO_API int neo_api_get_padsize(int * pad_size);
...
NEO_API int neo_ssl_server_certificate_verify(const byte* cert_pubkey_asn1,int pub_size,const byte * hash_cert,const byte * sign_asn1,int sign_size,int * pverify);
NEO_API int neo_ssl_server_key_exchange_set_peer_pubkey(const byte* peer_pubkey_asn1,int pub_size);
NEO_API int neo_ssl_server_key_exchange_verify(const byte* hash,const byte * sign_asn1,int sign_size,int* pverify);
NEO_API int neo_ssl_client_certificate(const byte * hash_cert,byte * sign_asn1,int * psign_size);
...
NEO_API int neo_ssl_client_encrypt(const byte * orgmsg,byte* out,int* pout_size);
NEO_API int neo_ssl_server_decrypt(const byte * orgmsg,byte* out,int* pout_size);
//END NEO_API_DEC
#ifdef __cplusplus
}   /* extern "C" */
#endif
