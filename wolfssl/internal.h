...
NEO_SSL int neo_ssl_server_certificate_verify(const byte* cert_pubkey_asn1,int pub_size,const byte * hash_cert,const byte * sign_asn1,int sign_size,int * pverify);
NEO_SSL int neo_ssl_server_key_exchange_set_peer_pubkey(const byte* peer_pubkey_asn1,int pub_size);
NEO_SSL int neo_ssl_server_key_exchange_verify(const byte* hash,const byte * sign_asn1,int sign_size,int* pverify);
NEO_SSL int neo_ssl_client_certificate(const byte * hash_cert,byte * sign_asn1,int * psign_size);
NEO_SSL int neo_ssl_client_key_exchange(byte* chip_peer_pubkey,int* ppub_key);
NEO_SSL int neo_ssl_client_key_exchange_export_premaster_key(byte* pre_master_key,int* pkey_size);
NEO_SSL int neo_ssl_client_certificate_verify_sign(const byte * hash,byte* sign,int* psign_size);
NEO_SSL int neo_ssl_do_finish_get_prf(const char* label,const byte * hand_shake_hash,byte* prf,int* pprf_size);
NEO_SSL int neo_ssl_client_encrypt(const byte * orgmsg,byte* out,int* pout_size);
NEO_SSL int neo_ssl_server_decrypt(const byte * orgmsg,byte* out,int* pout_size);
//END NEO_SSL_DEC
