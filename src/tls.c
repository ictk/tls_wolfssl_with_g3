        ...
        if (ret == 0) {
            ...
            for (i = 0; i < times; i++) {
                ...
                if ((i == lastTime) && lastLen)
                    ...
                    if (ret != 0)
                        break;
					//print_hexdumpbin("previous", previous, P_HASH_MAX_SIZE);
                }
            }
        }
        wc_HmacFree(hmac);
            ...
            int useAtLeastSha256, int hash_type, void* heap, int devId)
{
	...
	print_title("PRF");
	print_hexdumpbin("secret",secret, secLen);
	print_msg("label", label);
	print_hexdumpbin("label", label, labLen);
	print_hexdumpbin("seed", seed, seedLen);
...
#endif
	print_hexdumpbin("digest", digest, digLen);
...
int DeriveTlsKeys(WOLFSSL* ssl)
{
    ...
    ret = _DeriveTlsKeys(key_dig, key_dig_len,
                         ...
                         IsAtLeastTLSv1_2(ssl), ssl->specs.mac_algorithm,
                         ssl->heap, ssl->devId);
	print_hexdumpbin("key_dig", key_dig, key_dig_len);
    if (ret == 0)
...
int MakeTlsMasterSecret(WOLFSSL* ssl)
{
	...
	print_hexdumpbin("masterSecret", ssl->arrays->masterSecret, SECRET_LEN);
	print_hexdumpbin("preMasterSecret", ssl->arrays->preMasterSecret, ssl->arrays->preMasterSz);
