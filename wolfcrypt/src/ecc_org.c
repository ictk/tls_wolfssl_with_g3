...
{
	//print_title("_wc_ecc_curve_free");
    if (curve == NULL) {
        return;
    }
...
{
	//print_title("wc_ecc_curve_load_item");
    int err;
...
#ifdef ECC_CACHE_CURVE
    x = wc_ecc_get_curve_idx_org(dp->id);
...
const char* wc_ecc_get_name_org(int curve_id)
{
	print_title("wc_ecc_get_name_org");
	
    int curve_idx = wc_ecc_get_curve_idx_org(curve_id);
...
int wc_ecc_set_curve_org(ecc_key* key, int keysize, int curve_id)
{
	print_title("wc_ecc_set_curve_org");
...
*/
int ecc_projective_add_point_org(ecc_point* P, ecc_point* Q, ecc_point* R,
                             mp_int* a, mp_int* modulus, mp_digit mp)
{
	//print_title("ecc_projective_add_point_org");
   ...
   if (err == MP_OKAY) {
       if ( (mp_cmp(P->x, Q->x) == MP_EQ) &&
           ...
           mp_clear(&t1);
           mp_clear(&t2);
          return ecc_projective_dbl_point_org(P, R, a, modulus, mp);
       }
   }
...
/**
   ...
   return   MP_OKAY on success
*/
int ecc_projective_dbl_point_org(ecc_point *P, ecc_point *R, mp_int* a,
                                       mp_int* modulus, mp_digit mp)
{
	//print_title("ecc_projective_dbl_point_org");
...
/**
  ...
  return   MP_OKAY on success
*/
int ecc_map_org(ecc_point* P, mp_int* modulus, mp_digit mp)
{
	//print_title("ecc_map_org");
...
static int normal_ecc_mulmod(mp_int* k, ecc_point *G, ecc_point *R,
                  ...
                  void* heap)
#else
int wc_ecc_mulmod_ex_org(mp_int* k, ecc_point *G, ecc_point *R,
                  mp_int* a, mp_int* modulus, int map,
                  void* heap)
#endif
{
	//print_title("wc_ecc_mulmod_ex_org");
  ...
  for (i = 0; i < M_POINTS; i++) {
      M[i] = wc_ecc_new_point_h_org(heap);
      if (M[i] == NULL) {
         mp_clear(&mu);
   ...
   tG = wc_ecc_new_point_h_org(heap);
   if (tG == NULL)
       err = MEMORY_E;
   ...
   /* M[0] == 8G */
   if (err == MP_OKAY)
       err = ecc_projective_dbl_point_org(tG, M[0], a, modulus, mp);
   if (err == MP_OKAY)
       err = ecc_projective_dbl_point_org(M[0], M[0], a, modulus, mp);
   if (err == MP_OKAY)
       err = ecc_projective_dbl_point_org(M[0], M[0], a, modulus, mp);
   ...
   if (err == MP_OKAY)
       for (j = 9; j < 16; j++) {
           err = ecc_projective_add_point_org(M[j-9], tG, M[j-M_POINTS], a,
                                                                modulus, mp);
   ...
   if (err == MP_OKAY) {
       ...
       for (;;) {
           ...
           if (mode == 1 && i == 0) {
               err = ecc_projective_dbl_point_org(R, R, a, modulus, mp);
               if (err != MP_OKAY) break;
               continue;
           }
           ...
           if (bitcpy == WINSIZE) {
               ...
               if (first == 1) {
                   ...
                   err = ecc_projective_add_point_org(R, M[bitbuf-M_POINTS], R, a,
                                                               modulus, mp);
               }
               if (err != MP_OKAY) break;
           ...
           for (j = 0; j < bitcpy; j++) {
               ...
               if ((bitbuf & (1 << WINSIZE)) != 0) {
                   if (first == 1) {
                       ...
                       /* then add */
                       err = ecc_projective_add_point_org(R, tG, R, a, modulus,
                                                                       mp);
                       if (err != MP_OKAY) break;
                   }
               }
   ...
   /* M[1] == 2G */
   if (err == MP_OKAY)
       err = ecc_projective_dbl_point_org(tG, M[1], a, modulus, mp);
   ...
   if (err == MP_OKAY) {
       for (;;) {
           ...
           if (mode == 0 && i == 0) {
               ...
               if (err == MP_OKAY)
                   err = ecc_projective_add_point_org(M[0], M[1], M[2], a, modulus,
                                                  mp);
               if (err == MP_OKAY)
                   err = ecc_projective_dbl_point_org(M[1], M[2], a, modulus, mp);
               if (err == MP_OKAY)
                   continue;
           }
           ...
           if (err == MP_OKAY)
               err = ecc_projective_add_point_org(M[0], M[1], M[i^1], a, modulus,
                                                                       mp);
#ifdef WC_NO_CACHE_RESISTANT
           if (err == MP_OKAY)
               err = ecc_projective_dbl_point_org(M[i], M[i], a, modulus, mp);
#else
           ...
           if (err == MP_OKAY)
               err = mp_copy((mp_int*)
                             ...
                             M[2]->z);
           if (err == MP_OKAY)
               err = ecc_projective_dbl_point_org(M[2], M[2], a, modulus, mp);
   ...
   if (err == MP_OKAY && map)
       err = ecc_map_org(R, modulus, mp);

exit:

   /* done */
   wc_ecc_del_point_h_org(tG, heap);
   for (i = 0; i < M_POINTS; i++) {
       wc_ecc_del_point_h_org(M[i], heap);
   }
...
*/
int wc_ecc_mulmod_org(mp_int* k, ecc_point *G, ecc_point *R, mp_int* a,
                  mp_int* modulus, int map)
{
	//print_title("wc_ecc_mulmod_org");

    return wc_ecc_mulmod_ex_org(k, G, R, a, modulus, map, NULL);
}
 ...
 */
ecc_point* wc_ecc_new_point_h_org(void* heap)
{
	//print_title("wc_ecc_new_point_h_org");
...
*/
ecc_point* wc_ecc_new_point_org(void)
{
	print_title("wc_ecc_new_point_org");

  return wc_ecc_new_point_h_org(NULL);
}


void wc_ecc_del_point_h_org(ecc_point* p, void* heap)
{
	//print_title("wc_ecc_del_point_h_org");
...
void wc_ecc_del_point_org(ecc_point* p)
{
	//print_title("wc_ecc_del_point_org");

    wc_ecc_del_point_h_org(p, NULL);
}
...
int wc_ecc_copy_point_org(ecc_point* p, ecc_point *r)
{
	print_title("wc_ecc_copy_point_org");
...
int wc_ecc_cmp_point_org(ecc_point* a, ecc_point *b)
{
	print_title("wc_ecc_cmp_point_org");
...
int wc_ecc_is_valid_idx_org(int n)
{
	//print_title("wc_ecc_is_valid_idx_org");
	   int x;
...
int wc_ecc_get_curve_idx_org(int curve_id)
{
	print_title("wc_ecc_get_curve_idx_org");
	    int curve_idx;
...
int wc_ecc_get_curve_id_org(int curve_idx)
{
	print_title("wc_ecc_get_curve_id_org");
	    if (wc_ecc_is_valid_idx_org(curve_idx)) {
...
int wc_ecc_get_curve_size_from_id_org(int curve_id)
{
	//print_title("wc_ecc_get_curve_size_from_id_org");
	    int curve_idx = wc_ecc_get_curve_idx_org(curve_id);
    if (curve_idx == ECC_CURVE_INVALID)
        return ECC_BAD_ARG_E;
...
int wc_ecc_get_curve_idx_from_name_org(const char* curveName)
{
	//print_title("wc_ecc_get_curve_idx_from_name_org");
	    int curve_idx;
    word32 len;
...
int wc_ecc_get_curve_size_from_name_org(const char* curveName)
{
	print_title("wc_ecc_get_curve_size_from_name_org");
	    int curve_idx;

    if (curveName == NULL)
        return BAD_FUNC_ARG;

    curve_idx = wc_ecc_get_curve_idx_from_name_org(curveName);
    if (curve_idx < 0)
        return curve_idx;
...
int wc_ecc_get_curve_id_from_name_org(const char* curveName)
{
	print_title("wc_ecc_get_curve_id_from_name_org");
	    int curve_idx;
 ...
 * return curve id, from ecc_sets[] on success, negative on error
 */
int wc_ecc_get_curve_id_from_params_org(int fieldSize,
        const byte* prime, word32 primeSz, const byte* Af, word32 AfSz,
        const byte* Bf, word32 BfSz, const byte* order, word32 orderSz,
        const byte* Gx, word32 GxSz, const byte* Gy, word32 GySz, int cofactor)
{
	print_title("wc_ecc_get_curve_id_from_params_org");
	    int idx;
    int curveSz;
...
/**
  ...
  return           MP_OKAY if successful
*/
int wc_ecc_shared_secret_org(ecc_key* private_key, ecc_key* public_key, byte* out,
                      word32* outlen)
{
   ...
   if (wc_ecc_is_valid_idx_org(private_key->idx) == 0 ||
       wc_ecc_is_valid_idx_org(public_key->idx)  == 0) {
      return ECC_BAD_ARG_E;
   }
   ...
   ret = mp_to_unsigned_bin(public_key->pubkey.x, tmppubkey);
   print_hexdumpbin("public_key X", tmppubkey, tmppubkey_size);
   ...
   *outlen = private_key->dp->size;
   ...
   err = wc_ecc_shared_secret_ex_org(private_key, &public_key->pubkey, out, outlen);
#endif /* WOLFSSL_ATECC508A */
   print_hexdumpbin("wc_ecc_shared_secret_org OUT", out,*outlen);
   return err;
}
                               ...
                               byte* out, word32* outlen, ecc_curve_spec* curve)
{
    ...
    {
        /* make new point */
        result = wc_ecc_new_point_h_org(private_key->heap);
        if (result == NULL) {
#ifdef HAVE_ECC_CDH
            if (k == &k_lcl)
                mp_clear(k);
        ...
        err = wc_ecc_mulmod_ex_org(k, point, result, curve->Af, curve->prime, 1,
                                                             private_key->heap);
        if (err == MP_OKAY) {
        ...
        *outlen = x;
        ...
        wc_ecc_del_point_h_org(result, private_key->heap);
    }
#ifdef HAVE_ECC_CDH
    if (k == &k_lcl)
        mp_clear(k);
...
int wc_ecc_shared_secret_gen_org(ecc_key* private_key, ecc_point* point,
                                                    byte* out, word32 *outlen)
{
	print_title("wc_ecc_shared_secret_gen_org");
	    int err;
...
/**
 ...
 return           MP_OKAY if successful
*/
int wc_ecc_shared_secret_ex_org(ecc_key* private_key, ecc_point* point,
                            byte* out, word32 *outlen)
{
	print_title("wc_ecc_shared_secret_ex_org");
	    int err;
    ...
    switch(private_key->state) {
        ...
        case ECC_STATE_SHARED_SEC_GEN:
            ...
            err = wc_ecc_shared_secret_gen_org(private_key, point, out, outlen);
            if (err < 0) {
                break;
            }
            FALL_THROUGH;
...
int wc_ecc_point_is_at_infinity_org(ecc_point* p)
{
	print_title("wc_ecc_point_is_at_infinity_org");
	    if (p == NULL)
        return BAD_FUNC_ARG;
 ...
 *      curve type multiple times when generating a key with wc_ecc_make_key_org().
 *
        ...
        ecc_point* pubOut)
{
    ...
    {
        if (err == MP_OKAY) {
            base = wc_ecc_new_point_h_org(key->heap);
            if (base == NULL)
                err = MEMORY_E;
        }
        ...
        /* make the public key */
        if (err == MP_OKAY) {
            err = wc_ecc_mulmod_ex_org(&key->k, base, pub, curve->Af, curve->prime,
                                                                  1, key->heap);
        }

        wc_ecc_del_point_h_org(base, key->heap);
    }
...
int wc_ecc_make_pub_org(ecc_key* key, ecc_point* pubOut)
{
	print_title("wc_ecc_make_pub_org");
	    WOLFSSL_ENTER("wc_ecc_make_pub_org");
...
int wc_ecc_make_key_ex_org(WC_RNG* rng, int keysize, ecc_key* key, int curve_id)
{
	print_title("wc_ecc_make_key_ex_org");
	    int            err;
#ifndef WOLFSSL_ATECC508A
    DECLARE_CURVE_SPECS(ECC_CURVE_FIELD_COUNT)
#endif
    ...
    err = wc_ecc_set_curve_org(key, keysize, curve_id);
    if (err != 0) {
        return err;
    }
...
int wc_ecc_make_key_org(WC_RNG* rng, int keysize, ecc_key* key)
{
	print_title("wc_ecc_make_key_org");
	    return wc_ecc_make_key_ex_org(rng, keysize, key, ECC_CURVE_DEF);
}
...
int wc_ecc_init_ex_org(ecc_key* key, void* heap, int devId)
{
	print_title("wc_ecc_init_ex_org");
	    int ret = 0;
...
int wc_ecc_init_org(ecc_key* key)
{
	print_title("wc_ecc_init_org");
	    return wc_ecc_init_ex_org(key, NULL, INVALID_DEVID);
}

int wc_ecc_set_flags_org(ecc_key* key, word32 flags)
{
	print_title("wc_ecc_set_flags_org");
	    if (key == NULL) {
        return BAD_FUNC_ARG;
    }
    key->flags |= flags;
...
/**
 ...
 return    MP_OKAY if successful
 */
int wc_ecc_sign_hash_org(const byte* in, word32 inlen, byte* out, word32 *outlen,
                     WC_RNG* rng, ecc_key* key)
{
	print_title("wc_ecc_sign_hash_org");
	print_hexdumpbin("in",in,inlen);
    ...
    switch(key->state) {
        ...
        case ECC_STATE_SIGN_DO:
            ...
            err = wc_ecc_sign_hash_ex_org(in, inlen, rng, key, r, s);
            if (err < 0) {
                break;
            }
        ...
        case ECC_STATE_SIGN_ENCODE:
			...
			print_hexdumpbin("r", r->dp, r->used*4);
			print_hexdumpbin("s", s->dp, s->used * 4);

			print_intarray("r", r->dp, r->used);
			print_intarray("s", s->dp, s->used);
			{
				byte buff[64];
				memset(buff, 0, 64);
				int size = mp_to_unsigned_bin(r, buff);
				print_hexdumpbin("r just hex", buff, 32);

				memset(buff, 0, 64);
				size = mp_to_unsigned_bin(s, buff);
	...
	//print_hexdumpbin("r", r->, *outlen);
	//print_hexdumpbin("s", out, *outlen);
	print_hexdumpbin("out", out, *outlen);
    /* cleanup */
...
/**
  ...
  return    MP_OKAY if successful
*/
int wc_ecc_sign_hash_ex_org(const byte* in, word32 inlen, WC_RNG* rng,
                     ecc_key* key, mp_int *r, mp_int *s)
{
	print_title("wc_ecc_sign_hash_ex_org");
	   int    err;
   mp_int e;
   ...
   /* is the IDX valid ?  */
   if (wc_ecc_is_valid_idx_org(key->idx) != 1) {
      return ECC_BAD_ARG_E;
   }
   ...
   if (err == MP_OKAY) {
       ...
       if (wc_ecc_init_ex_org(&pubkey, key->heap, INVALID_DEVID) == MP_OKAY) {
       #ifdef WOLFSSL_CUSTOM_CURVES
           ...
           for (;;) {
               ...
               err = wc_ecc_make_key_ex_org(rng, key->dp->size, &pubkey,
                                                              key->dp->id);
               if (err != MP_OKAY) break;
...
#endif
{
  ...
  if (err == MP_OKAY) {
    ...
    if (err == MP_OKAY) {
        for (x = 0; x < 16; x++) {
            precomp[x] = wc_ecc_new_point_h_org(heap);
            if (precomp[x] == NULL) {
                err = GEN_MEM_ERR;
                break;
            }
  ...
  if (err == MP_OKAY)
    ...
    err = ecc_projective_dbl_point_org(precomp[1], precomp[2], a, modulus, mp);

  if (err == MP_OKAY)
    err = ecc_projective_add_point_org(precomp[1], precomp[2], precomp[3],
                                   a, modulus, mp);
  if (err == MP_OKAY)
    /* precomp [0,i](A + B) table */
    err = ecc_projective_dbl_point_org(precomp[1<<2], precomp[2<<2], a, modulus, mp);

  if (err == MP_OKAY)
    err = ecc_projective_add_point_org(precomp[1<<2], precomp[2<<2], precomp[3<<2],
                                   a, modulus, mp);
  ...
  if (err == MP_OKAY) {
    ...
    for (x = 1; x < 4; x++) {
        for (y = 1; y < 4; y++) {
            if (err == MP_OKAY)
                err = ecc_projective_add_point_org(precomp[x], precomp[(y<<2)],
  ...
  if (err == MP_OKAY) {
    ...
    for (x = 0;; ) {
        ...
        if (first == 0) {
            /* double twice */
            if (err == MP_OKAY)
                err = ecc_projective_dbl_point_org(C, C, a, modulus, mp);
            if (err == MP_OKAY)
                err = ecc_projective_dbl_point_org(C, C, a, modulus, mp);
            else
                break;
        ...
        if ((nA != 0) || (nB != 0)) {
            if (first == 1) {
                ...
                if (err == MP_OKAY)
                    err = ecc_projective_add_point_org(C, precomp[nA + (nB<<2)], C,
                                                   a, modulus, mp);
                else
                    break;
            }
  ...
  /* reduce to affine */
  if (err == MP_OKAY)
    err = ecc_map_org(C, modulus, mp);

  /* clean up */
  for (x = 0; x < 16; x++) {
     wc_ecc_del_point_h_org(precomp[x], heap);
  }
 ...
 */
int wc_ecc_verify_hash_org(const byte* sig, word32 siglen, const byte* hash,
                       word32 hashlen, int* res, ecc_key* key)
{
	print_title("wc_ecc_verify_hash_org");
	print_hexdumpbin("sig", sig, siglen);
	print_hexdumpbin("hash", hash, hashlen);
    ...
    switch(key->state) {
        ...
        case ECC_STATE_VERIFY_DO:
            ...
            err = wc_ecc_verify_hash_ex_org(r, s, hash, hashlen, res, key);
...
/**
   ...
   return      MP_OKAY if successful (even if the signature is not valid)
*/
int wc_ecc_verify_hash_ex_org(mp_int *r, mp_int *s, const byte* hash,
                    word32 hashlen, int* res, ecc_key* key)
{
	print_title("wc_ecc_verify_hash_ex_org");
	   int           err;
#ifndef WOLFSSL_ATECC508A
   int          did_init = 0;
  ...
  if (key->type == ECC_PRIVATEKEY_ONLY) {
   ...
   /* allocate points */
   if (err == MP_OKAY) {
       mG = wc_ecc_new_point_h_org(key->heap);
       mQ = wc_ecc_new_point_h_org(key->heap);
       if (mQ  == NULL || mG == NULL)
          err = MEMORY_E;
   }
   ...
   if (err == MP_OKAY)
       err = wc_ecc_mulmod_ex_org(&u1, mG, mG, curve->Af, curve->prime, 0, key->heap);
   if (err == MP_OKAY)
       err = wc_ecc_mulmod_ex_org(&u2, mQ, mQ, curve->Af, curve->prime, 0, key->heap);
   if (err == MP_OKAY)
...
#ifndef ECC_SHAMIR
    {
        ...
        if (err == MP_OKAY) {
            err = wc_ecc_mulmod_ex_org(&u1, mG, mG, curve->Af, curve->prime, 0,
                                                                     key->heap);
        }
        if (err == MP_OKAY) {
            err = wc_ecc_mulmod_ex_org(&u2, mQ, mQ, curve->Af, curve->prime, 0,
                                                                     key->heap);
        }
        ...
        /* add them */
        if (err == MP_OKAY)
            err = ecc_projective_add_point_org(mQ, mG, mG, curve->Af,
                                                             curve->prime, mp);

        /* reduce */
        if (err == MP_OKAY)
            err = ecc_map_org(mG, curve->prime, mp);
    }
#else
   ...
   /* cleanup */
   wc_ecc_del_point_h_org(mG, key->heap);
   wc_ecc_del_point_h_org(mQ, key->heap);
...
int wc_ecc_import_point_der_org(byte* in, word32 inLen, const int curve_idx,
                            ecc_point* point)
{
	print_title("wc_ecc_import_point_der_org");
	    int err = 0;
    int compressed = 0;

    if (in == NULL || point == NULL || (curve_idx < 0) ||
        (wc_ecc_is_valid_idx_org(curve_idx) == 0))
        return ECC_BAD_ARG_E;
...
/* export point to der */
int wc_ecc_export_point_der_org(const int curve_idx, ecc_point* point, byte* out,
                            word32* outLen)
{
	print_title("wc_ecc_export_point_der_org");
	    int    ret = MP_OKAY;
    word32 numlen;
#ifndef WOLFSSL_ATECC508A
#ifdef WOLFSSL_SMALL_STACK
    ...
    if ((curve_idx < 0) || (wc_ecc_is_valid_idx_org(curve_idx) == 0))
        return ECC_BAD_ARG_E;
...
int wc_ecc_export_x963_org(ecc_key* key, byte* out, word32* outLen)
{
	print_title("wc_ecc_export_x963_org");
	   int    ret = MP_OKAY;
   word32 numlen;
#ifndef WOLFSSL_ATECC508A
#ifdef WOLFSSL_SMALL_STACK
   ...
   if (wc_ecc_is_valid_idx_org(key->idx) == 0) {
      return ECC_BAD_ARG_E;
   }
   numlen = key->dp->size;
...
#endif
#endif /* WOLFSSL_ATECC508A */
   print_hexdumpbin("wc_ecc_export_x963_org out", out, *outLen);
 ...
 * compression option */
int wc_ecc_export_x963_ex_org(ecc_key* key, byte* out, word32* outLen,
                          int compressed)
{
	print_title("wc_ecc_export_x963_ex_org");
	    if (compressed == 0)
        return wc_ecc_export_x963_org(key, out, outLen);
#ifdef HAVE_COMP_KEY
    else
...
int wc_ecc_is_point_org(ecc_point* ecp, mp_int* a, mp_int* b, mp_int* prime)
{
	print_title("wc_ecc_is_point_org");
	   int err;
   mp_int t1, t2;
...
{
	print_title("ecc_check_privkey_gen");
    int        err = MP_OKAY;
    ecc_point* base = NULL;
    ecc_point* res  = NULL;
    DECLARE_CURVE_SPECS(2)
    ...
    res = wc_ecc_new_point_h_org(key->heap);
    if (res == NULL)
        err = MEMORY_E;
...
#endif
    {
        ...
        if (err == MP_OKAY)
            err = wc_ecc_mulmod_ex_org(&key->k, base, res, a, prime, 1, key->heap);
    }
    ...
    wc_ecc_curve_free(curve);
    wc_ecc_del_point_h_org(res, key->heap);
    wc_ecc_del_point_h_org(base, key->heap);
        ...
        mp_int* prime, mp_int* order)
{
    ...
    inf = wc_ecc_new_point_h_org(key->heap);
    if (inf == NULL)
        err = MEMORY_E;
    else {
#ifdef WOLFSSL_HAVE_SP_ECC
...
#endif
#endif
            err = wc_ecc_mulmod_ex_org(order, pubkey, inf, a, prime, 1, key->heap);
        if (err == MP_OKAY && !wc_ecc_point_is_at_infinity_org(inf))
            err = ECC_INF_E;
    }

    wc_ecc_del_point_h_org(inf, key->heap);
...
int wc_ecc_check_key_org(ecc_key* key)
{
	print_title("wc_ecc_check_key_org");
	    int    err;
#ifndef WOLFSSL_ATECC508A
    mp_int* b;
#ifdef USE_ECC_B_PARAM
    ...
    if (wc_ecc_point_is_at_infinity_org(&key->pubkey))
        return ECC_INF_E;
    ...
    if (err == MP_OKAY)
        err = wc_ecc_is_point_org(&key->pubkey, curve->Af, b, curve->prime);
...
int wc_ecc_import_x963_ex_org(const byte* in, word32 inLen, ecc_key* key,
                          int curve_id)
{
	print_title("wc_ecc_import_x963_ex_org");
	print_hexdumpbin("wc_ecc_import_x963_ex_org in", in, inLen);
	    int err = MP_OKAY;
#ifndef WOLFSSL_ATECC508A
    int compressed = 0;
#endif /* !WOLFSSL_ATECC508A */
...
int wc_ecc_import_x963_org(const byte* in, word32 inLen, ecc_key* key)
{
	print_title("wc_ecc_import_x963_org");
	    return wc_ecc_import_x963_ex_org(in, inLen, key, ECC_CURVE_DEF);
}
#endif /* HAVE_ECC_KEY_IMPORT */
...
int wc_ecc_export_private_only_org(ecc_key* key, byte* out, word32* outLen)
{
	print_title("wc_ecc_export_private_only_org");
	    word32 numlen;
		int ret;
...
#else
	ret = mp_to_unsigned_bin(&key->k, out + (numlen -
		mp_unsigned_bin_size(&key->k)));
	print_hexdumpbin("wc_ecc_export_private_only_org out", out, *outLen);
	return ret;
#endif /* WOLFSSL_ATECC508A */
}
...
int wc_ecc_export_public_raw_org(ecc_key* key, byte* qx, word32* qxLen,
                             byte* qy, word32* qyLen)
{
	print_title("wc_ecc_export_public_raw_org");
 ...
 * return MP_OKAY on success, negative on error */
int wc_ecc_export_private_raw_org(ecc_key* key, byte* qx, word32* qxLen,
                              byte* qy, word32* qyLen, byte* d, word32* dLen)
{
	print_title("wc_ecc_export_private_raw_org");
...
int wc_ecc_import_private_key_ex_org(const byte* priv, word32 privSz,
                                 const byte* pub, word32 pubSz, ecc_key* key,
                                 int curve_id)
{
	print_title("wc_ecc_import_private_key_ex_org");

	print_hexdumpbin("priv", priv, privSz);
	print_hexdumpbin("pub", pub, pubSz);
	    int ret;

    /* public optional, NULL if only importing private */
    if (pub != NULL) {
        ret = wc_ecc_import_x963_ex_org(pub, pubSz, key, curve_id);
        key->type = ECC_PRIVATEKEY;
    }
    else {
        ...
        /* set key size */
        ret = wc_ecc_set_curve_org(key, privSz, curve_id);
        key->type = ECC_PRIVATEKEY_ONLY;
    }
	...
	print_intarray("key->k", key->k.dp, key->k.used);
...
int wc_ecc_import_private_key_org(const byte* priv, word32 privSz, const byte* pub,
                           word32 pubSz, ecc_key* key)
{
	print_title("wc_ecc_import_private_key_org");
	    return wc_ecc_import_private_key_ex_org(priv, privSz, pub, pubSz, key,
                                                                ECC_CURVE_DEF);
}
#endif /* HAVE_ECC_KEY_IMPORT */
...
int wc_ecc_rs_to_sig_org(const char* r, const char* s, byte* out, word32* outlen)
{
	print_title("wc_ecc_rs_to_sig_org");
	    int err;
    mp_int rtmp;
    mp_int stmp;
...
/**
   ...
   return  MP_OKAY on success, negative on error
*/
int wc_ecc_sig_to_rs_org(const byte* sig, word32 sigLen, byte* r, word32* rLen,
                     byte* s, word32* sLen)
{
	print_title("wc_ecc_sig_to_rs_org");
	    int err;
    word32 x = 0;
    mp_int rtmp;
    mp_int stmp;
          ...
          const char* qy, const char* d, int curve_id)
{
    ...
    err = wc_ecc_set_curve_org(key, 0, curve_id);
    if (err != 0) {
        return err;
    }
...
/**
   ...
   return    MP_OKAY on success
*/
int wc_ecc_import_raw_ex_org(ecc_key* key, const char* qx, const char* qy,
                   const char* d, int curve_id)
{
	print_title("wc_ecc_import_raw_ex_org");
...
/**
   ...
   return    MP_OKAY on success
*/
int wc_ecc_import_raw_org(ecc_key* key, const char* qx, const char* qy,
                   const char* d, const char* curveName)
{
	print_title("wc_ecc_import_raw_org");
	    int err, x;
...
/* worst case estimate, check actual return from wc_ecc_sign_hash_org for actual
   value of signature size in octets */
int wc_ecc_sig_size_org(ecc_key* key)
{
	print_title("wc_ecc_sig_size_org");
	    int sz = wc_ecc_size_org(key);
    if (sz <= 0)
        return sz;
...
static int find_hole(void)
{
   ...
   if (z >= 0 && fp_cache[z].g) {
      ...
      wc_ecc_del_point_org(fp_cache[z].g);
      fp_cache[z].g  = NULL;
      for (x = 0; x < (1U<<FP_LUT); x++) {
         wc_ecc_del_point_org(fp_cache[z].LUT[x]);
...
static int add_entry(int idx, ecc_point *g)
{
   ...
   fp_cache[idx].g = wc_ecc_new_point_org();
   ...
   if ((mp_copy(g->x, fp_cache[idx].g->x) != MP_OKAY) ||
      ...
      wc_ecc_del_point_org(fp_cache[idx].g);
      fp_cache[idx].g = NULL;
      return GEN_MEM_ERR;
   }

   for (x = 0; x < (1U<<FP_LUT); x++) {
      fp_cache[idx].LUT[x] = wc_ecc_new_point_org();
      if (fp_cache[idx].LUT[x] == NULL) {
         for (y = 0; y < x; y++) {
            wc_ecc_del_point_org(fp_cache[idx].LUT[y]);
    ...
    mp_int* mu)
{
   ...
   for (x = 1; x < FP_LUT; x++) {
      ...
      if ((mp_copy(fp_cache[idx].LUT[1<<(x-1)]->x,
         ...
         for (y = 0; y < lut_gap; y++) {
             if ((err = ecc_projective_dbl_point_org(fp_cache[idx].LUT[1<<x],
   ...
   for (x = 2; x <= FP_LUT; x++) {
       ...
       for (y = 0; y < (1UL<<FP_LUT); y++) {
           ...
           /* perform the add */
           if ((err = ecc_projective_add_point_org(
...
#define KB_SIZE 128
   ...
   if ((err = mp_to_unsigned_bin(&tk, kb)) == MP_OKAY) {
      ...
      for (x = lut_gap-1; x >= 0; x--) {
          ...
          /* double if not first */
          if (!first) {
             if ((err = ecc_projective_dbl_point_org(R, R, a, modulus,
                                                             mp)) != MP_OKAY) {
                break;
             }
          }
          ...
          if (!first && z) {
             if ((err = ecc_projective_add_point_org(R, fp_cache[idx].LUT[z], R,
...
#define KB_SIZE 128
   ...
   if ((err = mp_to_unsigned_bin(&tkb, kb[1])) == MP_OKAY) {
      ...
      for (x = lut_gap-1; x >= 0; x--) {
          ...
          if (!first) {
             if (zA) {
                if ((err = ecc_projective_add_point_org(R, fp_cache[idx1].LUT[zA],
                                                  R, a, modulus, mp)) != MP_OKAY) {
                   break;
                }
             }
             if (zB) {
                if ((err = ecc_projective_add_point_org(R, fp_cache[idx2].LUT[zB],
                                                  R, a, modulus, mp)) != MP_OKAY) {
                   break;
                }
             }
             ...
             if (zB && first == 0) {
                if (zB) {
                   if ((err = ecc_projective_add_point_org(R,
...
#undef KB_SIZE
   ...
   return ecc_map_org(R, modulus, mp);
}
...
/** ECC Fixed Point mulmod global
    ...
    return MP_OKAY if successful
*/
int wc_ecc_mulmod_ex_org(mp_int* k, ecc_point *G, ecc_point *R, mp_int* a,
...
static void wc_ecc_fp_free_cache(void)
{
   ...
   for (x = 0; x < FP_ENTRIES; x++) {
      if (fp_cache[x].g != NULL) {
         for (y = 0; y < (1U<<FP_LUT); y++) {
            wc_ecc_del_point_org(fp_cache[x].LUT[y]);
            fp_cache[x].LUT[y] = NULL;
         }
         wc_ecc_del_point_org(fp_cache[x].g);
         fp_cache[x].g         = NULL;
                ...
                word32 msgSz, byte* out, word32* outSz, ecEncCtx* ctx)
{
    ...
    do {
        ...
        if (ret != 0)
            break;
    #endif
        ret = wc_ecc_shared_secret_org(privKey, pubKey, sharedSecret, &sharedSz);
...
{
	print_title("wc_ecc_export_x963_compressed");
   word32 numlen;
   int    ret = MP_OKAY;
   ...
   *outLen = 1 + numlen;
   ...
   print_hexdumpbin("wc_ecc_export_x963_compressed", out, *outLen);
...
int wc_ecc_get_oid_org(word32 oidSum, const byte** oid, word32* oidSz)
{
	print_title("wc_ecc_get_oid_org");
	    int x;
